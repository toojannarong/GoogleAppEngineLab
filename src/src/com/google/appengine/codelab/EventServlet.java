...
public class EventServlet extends BaseServlet {
  ...
  private static final Logger logger = Logger.getLogger(EventServlet.class.getCanonicalName());
  /**
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    if (searchFor == null || searchFor.equals("") || searchFor == "*") {
      entities = Event.getAllProducts("Event");
      out.println(Util.writeJSON(entities));
    } else {
      Entity product = Event.getProduct(searchFor);
      if (product != null) {
  ...
  protected void doPut(HttpServletRequest req, HttpServletResponse resp)
    ...
    String dueDate = req.getParameter("description");
    try {
      Event.createOrUpdateProduct(category, dueDate);
    } catch (Exception e) {
  ...
  protected void doDelete(HttpServletRequest req, HttpServletResponse resp)
    ...
    PrintWriter out = resp.getWriter();
    try{    	
    	out.println(Event.deleteProduct(productkey));
    } catch(Exception e) {
